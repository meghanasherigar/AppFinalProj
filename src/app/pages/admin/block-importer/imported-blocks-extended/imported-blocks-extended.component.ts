...
export class ImportedBlocksExtendedComponent implements OnInit {
  ...
  setFormData() {
    ...
    this.blockImporterForm.controls["BlockType"].setValue(this.blockTypes.filter(id => id.blockType == "Other")[0]["blockTypeId"]);
    this.blockImporterForm.controls["BlockStatus"].setValue(this.blockStatus.filter(id => id.blockStatus == "Draft")[0]["blockStatusId"]);
    this.blockImporterForm.controls["TransactionType"].setValue(this.transactionList[0].transactionType);
    this.getIndustries(this.industryList);
  }
  ...
  getIndustries(data: Industry[]) {
    ...
    data.forEach(element => {
      ...
      if (!element.subIndustries || element.subIndustries.length == 0) {
        ...
        //     this.blockImporterForm.controls["BlockIndustryOthers"].setValue(selectedIndustry[0].industry);
        // }
      }
      else {
