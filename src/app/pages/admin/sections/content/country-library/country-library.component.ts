...
@Component({
  selector: 'ngx-country-library',
  templateUrl: './country-library.component.html',
  styleUrls: ['./country-library.component.scss']
})
export class CountryLibraryComponent implements OnInit, OnDestroy {

  constructor(private readonly _eventService: EventAggregatorService, private designerService: DesignerService, 
    private libraryService: LibraryService,private designerServiceProjectDesigner: DesignerServiceProjDesign,
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this._eventService.getEvent(eventConstantsEnum.projectDesigner.blockImporter.editBlockExtendedViewCountry).subscribe((payload: any) => {
      if (payload.section == RegionNames.TEMPLATE) {  
       ...
       this.DisplayLibrary = false;
       this.designerService.isExtendedIconicView = true;
       const dialogRef = this.dialogService.open(ImportBlocksProcessComponent, {
