...
import { BlockFilterDataModel, BlockFilterRequestDataModel, ActionOnBlockStack, StackModelFilter, templateFilterRequest } from '../../../../../../../../../@models/projectDesigner/block';
...
export class FilterTemplateComponent implements OnInit, OnDestroy {
  ...
  constructor(private blockService: BlockService, private service: IconViewService, private designerService: DesignerService,
    ...
    private el: ElementRef, protected ref: NbDialogRef<any>, private sharedService: ShareDetailService) { }
  ...
  ngOnInit() {
    ...
    this.templateBlockFilter.TemplateId = this.designer.templateDetails.templateId;
    ...
    let filterRequest = new templateFilterRequest();
    filterRequest.IsTemplate = true;
    filterRequest.TemplateId = this.designer.templateDetails.templateId;
  ...
  AssignPreviouslySelectedFilter()
  {
    ...
    if(currentDesignerService && currentDesignerService.templateDetails && 
      currentDesignerService.templateDetails.templateId === this.designer.templateDetails.templateId)
    {
    this.templateBlockFilter.TemplateId = currentDesignerService.templateDetails.templateId;
  ...
  apply() {
    ...
    if (this.designer.selectedFilterProjectYear.length == 0 && this.designer.selectedFilterBlockCreator.length == 0 && this.designer.selectedFilterblockOrigin.length == 0 && this.designer.selectedFilterblockState.length == 0
      && this.designer.selectedFilterblockStatus.length == 0 && this.designer.selectedFilterindustry.length == 0 && this.designer.selectedFiltertitle.length == 0
      && this.designer.selectedblockType.length == 0 && this.designer.selectedFilterDescription.length==0) {
    ...
    if (this.designer.selectedFilterProjectYear.length == 0 && this.designer.selectedFilterBlockCreator.length == 0 && this.designer.selectedFilterblockOrigin.length == 0 && this.designer.selectedFilterblockState.length == 0
      && this.designer.selectedFilterblockStatus.length == 0 && this.designer.selectedFilterindustry.length == 0 && this.designer.selectedFiltertitle.length == 0
      && this.designer.selectedFilterDescription.length==0
      ...
      this.blockService.blockSelectedFilter(this.templateBlockFilter).subscribe((response: any) => {
        ...
        payload.template = this.designer.templateDetails;
        payload.filterApplied = true;
        this.subscriptions.add(this._eventService.getEvent(this.section + "_loadTemplateContent").publish(payload));
        this.subscriptions.add(this._eventService.getEvent(this.section + "_templateFilterApplied").publish(true));
  ...
  clear() {
    ...
    this.selectedBlockType = this.designer.selectedblockType = [];
    this.saveThemeContext();
    this.subscriptions.add(this._eventService.getEvent(this.section + "_templateFilterApplied").publish(false));
  }
