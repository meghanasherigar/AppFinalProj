...
import { BlockDetailsResponseViewModel, BlockFilterDataModel, StackModelFilter } from '../../../../../../../../../@models/projectDesigner/block';
...
@Component({
  selector: 'ngx-theme1-library-header',
  templateUrl: './theme1-library-header.component.html',
  styleUrls: ['./theme1-library-header.component.scss']
})
export class Theme1LibraryHeaderComponent implements OnInit {
  ...
  constructor(private _eventService: EventAggregatorService, private sharedService: ShareDetailService, private libraryService: LibraryService, private designerService: DesignerService, private ngxLoader: NgxUiLoaderService,
  ...
  loadLibrary() {
    ...
    if (this.selectedIds.length == 0) {
      selectedData = (!this.themingContext.themeOptions[0].library) ? libraryList[0] : this.themingContext.themeOptions[0].library;
    ...
    if (this.section == ThemeSection.Theme1_1) {
  ...
  libraryChange1(event) {
    ...
    let selectedSection = selectedTheme.themeOptions.filter(id => id.name == ThemeSection.Theme1_1)[0];
  ...
  loadLibraryContent() {
    ...
    if (filterApplied.selectedFilterProjectYear.length > 0
      ...
      this.libraryService.getCategories().subscribe((data: BlockDetailsResponseViewModel[]) => {
        ...
        this.subscriptions.add(this._eventService.getEvent(this.section + "_" + eventConstantsEnum.projectDesigner.iconicViewTemplateSection.libraryDetails).publish(this.libraryBlockDetails));
