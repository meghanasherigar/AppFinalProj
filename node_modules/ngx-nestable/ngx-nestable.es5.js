import { ChangeDetectionStrategy, ChangeDetectorRef, Component, Directive, ElementRef, EventEmitter, HostListener, Input, NgModule, NgZone, Output, Renderer2, ViewEncapsulation } from '@angular/core';
import { CommonModule } from '@angular/common';
var _traverseChildren = function (tree, callback, parent) {
    if (parent === void 0) { parent = null; }
    for (var /** @type {?} */ i = 0; i < tree.length; i++) {
        var /** @type {?} */ item = tree[i];
        if (typeof item === 'undefined') {
            continue;
        }
        var /** @type {?} */ callbackResult = callback(item, parent);
        if (callbackResult) {
            break;
        }
        if (item.children) {
...
var _getParents = function (el, parentSelector) {
    ...
    var /** @type {?} */ parents = [];
    var /** @type {?} */ parentNode = el.parentNode;
    while (parentNode !== parentSelector) {
        var /** @type {?} */ o = parentNode;
        if (!parentNode) {
            break;
        }
...
var _closest = function (el, selector) {
    ...
    while (el) {
        ...
        var /** @type {?} */ matches = parent[matchesFn](selector);
        if (parent && matches) {
            return parent;
        }
        el = parent;
...
var hasPointerEvents = (function () {
    var /** @type {?} */ el = document.createElement('div'), /** @type {?} */ docEl = document.documentElement;
    ...
    var /** @type {?} */ supports = window.getComputedStyle &&
...
var NestableComponent = (function () {
    ...
    NestableComponent.prototype.ngOnInit = function () {
        ...
        var /** @type {?} */ optionKeys = Object.keys(defaultSettings);
    ...
    NestableComponent.prototype._calculateDepth = function () {
        ...
        var /** @type {?} */ depth;
        var /** @type {?} */ items = this.dragEl.querySelectorAll(this.options.itemNodeName);
    ...
    NestableComponent.prototype._showMasks = function () {
        var /** @type {?} */ masks = this.el.nativeElement.getElementsByClassName('nestable-item-mask');
        for (var /** @type {?} */ i = 0; i < masks.length; i++) {
    ...
    NestableComponent.prototype._move = function (event) {
        var /** @type {?} */ depth, /** @type {?} */ list;
        var /** @type {?} */ dragRect = this.dragEl.getBoundingClientRect();
        this.renderer.setStyle(this.dragEl, 'left', event.pageX - this._mouse.offsetX + PX);
        this.renderer.setStyle(this.dragEl, 'top', event.pageY - this._mouse.offsetY + PX);
        this._mouseUpdate(event);
        // axis mouse is now moving on
        var /** @type {?} */ newAx = Math.abs(this._mouse.distX) > Math.abs(this._mouse.distY) ? 1 : 0;
        ...
        var /** @type {?} */ pointEl = document.elementFromPoint(event.pageX - document.body.scrollLeft, event.pageY - (window.pageYOffset || document.documentElement.scrollTop));
        if (!hasPointerEvents) {
        ...
        if (!this.options.fixedDepth &&
            ...
            this._mouse.distAxX = 0;
            var /** @type {?} */ previous = this._placeholder.previousElementSibling;
            ...
            if (this._mouse.distX < 0) {
                ...
                var /** @type {?} */ next = document.querySelector("." + this.options.placeClass + " + " + this.options.itemNodeName);
                var /** @type {?} */ parentElement = this._placeholder.parentElement;
                if (!next && parentElement) {
                    var /** @type {?} */ closestItem = _closest(this._placeholder, this.options.itemNodeName);
                    if (closestItem) {
        ...
        var /** @type {?} */ pointElRoot = _closest(this.pointEl, "." + this.options.rootClass), /** @type {?} */ isNewRoot = pointElRoot
        ...
        if (!this._mouse.dirAx || isNewRoot) {
            ...
            var /** @type {?} */ before = event.pageY <
                _offset(this.pointEl).top + this.pointEl.clientHeight / 2;
            var /** @type {?} */ placeholderParent = this._placeholder.parentNode;
            // get point element depth
            var /** @type {?} */ pointRelativeDepth = void 0;
            ...
            if (this.options.fixedDepth) {
                if (pointRelativeDepth === this.relativeDepth - 1) {
                    var /** @type {?} */ childList = this.pointEl.querySelector(this.options.listNodeName);
    ...
    NestableComponent.prototype.reset = function () {
        var /** @type {?} */ keys = Object.keys(this._mouse);
    ...
    NestableComponent.prototype.dragStart = function (event, item, parentList) {
        ...
        if (!this.options.disableDrag) {
            ...
            var /** @type {?} */ dragItem = _closest(event.target, this.options.itemNodeName);
            if (dragItem === null) {
                return;
            }
            this._parentDragId = Number.parseInt(dragItem.parentElement.parentElement.id);
            var /** @type {?} */ dragRect = dragItem.getBoundingClientRect();
            this._showMasks();
    ...
    NestableComponent.prototype.dragStop = function (event) {
        ...
        this._hideMasks();
        if (this.dragEl) {
            var /** @type {?} */ draggedId = Number.parseInt(this.dragEl.firstElementChild.id);
            var /** @type {?} */ placeholderContainer = _closest(this._placeholder, this.options.itemNodeName);
            var /** @type {?} */ changedElementPosition = this._dragIndex !==
                Array.prototype.indexOf.call(this._placeholder.parentElement.children, this._placeholder);
            var /** @type {?} */ index = Array.prototype.indexOf.call(this._placeholder.parentElement.children, this._placeholder);
...
var NestableDragHandleDirective = (function () {
    /**
     * @param {?} _el
     */
...
var NestableExpandCollapseDirective = (function () {
    /**
     * @param {?} _el
     */
